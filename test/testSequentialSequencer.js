...
describe("Sequential Sequencer", function() {
  ...
  it("should have updated playback order on button press", function() {
    level = state.levels.byId["level_4"];
    segment = state.segments.byId[create_segmentId(level.levelId, 0)];
    sequencer =
      state.sequencers[getSegmentIdToSequencerId(state)[segment.segmentId]];
  ...
  it("should have updated playback order on 2nd button press", function() {
    ...
    expect(sequencer.bufSequence[1]).to.equal(
      ...
      "new bufSequence should have been inserted into second slot"
    );
    expect(sequencer.bufSequence[0]).to.equal(
      segmentIdToBufName[segment.segmentId],
      "first slot of bufSequence should have remained"
    );
  });
